...
public class StockController(StockManagementService stockService) : ControllerBase
{
    ...
    /// </summary>
    /// <returns></returns>
    [HttpGet("{username}/{start}/{count}")]
    public IActionResult ViewStockTransactions(string username, int start, int count) =>
        _stockService.GetStockTransactions(username, start, count) is List<StockTransactionDto> dtos ?
        Ok(dtos) :
        BadRequest("Failed to fetch stock transactions!");

    /// <summary>
    /// Generates transactions summaries for each product.
    /// </summary>
    /// <returns></returns>
    [HttpGet("{username}/{reason}/{monthIdx}")]
    public IActionResult ViewStockTransactionSummaries(string username,string reason, int monthIdx) =>
        _stockService.GetStockTransactionsSummaries(username,reason, monthIdx) is List<StockTransactionSummary> dtos ?
        Ok(dtos) :
        BadRequest("Failed to fetch stock transactions!");
